...
internal class CredentialsFactory : CredentialsFactoryBase
{
    ...
    protected override ICredentials? CreateCredentials( Type credType )
    {
        if( credType == typeof( BingCredentials ) )
            return _config.EngineViewModel.MapCredentials?.BingCredentials;

        if( credType == typeof( GoogleCredentials ) )
            return _config.EngineViewModel.MapCredentials?.GoogleCredentials;

        if( credType == typeof( OpenStreetCredentials ) )
            return _config.EngineViewModel.MapCredentials?.OpenStreetCredentials;

        return credType == typeof( OpenTopoCredentials )
            ? _config.EngineViewModel.MapCredentials?.OpenTopoCredentials
            : null;
    }
}
